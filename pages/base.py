...
class FrameWindowGrid(FrameWindow):
    ...
    def set_row_size(self, rows: list):
        for item in range(len(rows)):
            self._frame.rowconfigure(item, weight=rows[item])
    
    def set_column_size(self, columns:list):
        for item in range(len(columns)):
            self._frame.columnconfigure(item, weight=columns[item])
...
class ComboBoxVideo(ComboBoxWidget):
    ...
    def selected_item(self) -> dict:
        ...
        if seleccion: return {"success": True, "url_video": seleccion['url_video']}
        else: return {"success": False}  

class ComboBoxVideoPack(ComboBoxVideo):
    def __init__(self, frame: tk.Frame, videos: list, position, expanded: bool, witdh: int) -> None:
        super().__init__(frame, videos)
        self._combo_box.pack(pady=20, side=position,expand=expanded, witdh=witdh)
