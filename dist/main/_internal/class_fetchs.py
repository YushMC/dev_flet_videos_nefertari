...
class RequestToGetAllVideosToDownload(Requests):
    def __init__(self, token, api_path):
        super().__init__()
            ...
            try:
                async with session.get(self.__api_path, headers=headers, ssl=self._ssl_context) as response:
                    if response.status == 200:
                        ...
                        filtrados = [item for item in self.__data_from_nefertari["trips"] if item.get("url_video")]
                        return {"success":True, "message": self.__data_from_nefertari["message"], "trips": filtrados}
                    else:
                        return {"success": False, "message": f"Error: {await response.text()}"}
            except Exception as e:
                return {"success": False, "message": f"Error: {e}"}
